...
for_task2 = from_csv_df7.select("primaryName", "birthYear").filter((f.col("birthYear") > 1900) & (f.col("birthYear") < 2000))
...
for_task3 = from_csv_df2.select("primaryTitle", "originalTitle", "runtimeMinutes").filter(f.col("runtimeMinutes") > 120)
# rab = from_csv_df2.select("primaryTitle", "originalTitle", "isAdult", "runtimeMinutes").filter(f.col("runtimeMinutes") > 40)
# rab.show(100)
...
Act_names_df.count()
Exp_Act_names_df = for1_task4_df.join(Act_names_df, on=for1_task4_df["nconst"] == Act_names_df["nconst"], how='left')
...
Exp_Films_df = Exp_Act_names_df.join(Tit_films_df, on=Exp_Act_names_df["tconst"] == Tit_films_df["tconst"], how='left')
# Exp_Films_df.show()
...
Add_adult_df = Regions_df.join(All_films_adult_df, on=Regions_df["titleId"] == All_films_adult_df["tconst"], how='left')
# Add_adult_df.show()
...
# path_to_saved_principals = 'D:\\DataSets_project\\Saved_files\\task4\\saved_principals'
# for_task4.write.csv(path_to_saved_principals, header=True, mode="overwrite")
# print("Saved to D:\\DataSets_project\\Saved_files\\task4\\saved_principals")
